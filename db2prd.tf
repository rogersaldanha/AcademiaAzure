...
#}
resource "azurerm_network_interface" "nicdbprd2" {
  name                = "nic-vm-dbprd-02"
  location            = var.location
  resource_group_name = azurerm_resource_group.lab.name
...
//Criar uma VM
resource "azurerm_windows_virtual_machine" "prdvm2" {
  name                = "vm-db-prd-02"
  resource_group_name = azurerm_resource_group.lab.name
  ...
  network_interface_ids = [
    azurerm_network_interface.nicdbprd2.id,
  ]
...
resource "azurerm_network_interface_backend_address_pool_association" "lbprd2" {
  network_interface_id    = azurerm_network_interface.nicdbprd2.id
  ip_configuration_name   = "ip-vm-dbprd-02"
